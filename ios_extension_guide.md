# iOS Share Extension 纯原生静默下载配置指�?
由于 iOS 项目的结构特殊�?(依赖 `.xcodeproj` �?`project.pbxproj` 文件配置)，我们无法直接在 Windows 环境中通过命令行生成一个“Share Extension 目标”。这必须�?Xcode 中完成�?
当您将代码推送到 GitHub，或者在 macOS 虚拟机上打开该项目时，请按照以下步骤添加静默下载逻辑�?
## 步骤 1：在 Xcode 中添�?Target
1. �?Xcode 打开 `ios/Runner.xcworkspace`�?2. 点击顶部�?`File` -> `New` -> `Target...`
3. 选择 `Share Extension`，命名为 `TwitterShareExtension`�?4. 在弹出的对话框中�?*一定不�?* 选择激�?scheme (Activate scheme? �?Cancel)�?
## 步骤 2：配�?App Group (极其重要)
为了�?Flutter 存的 Cookie 能被 Share Extension 读到�?1. �?Xcode 中，选中 `Runner` target -> `Signing & Capabilities` -> 点击 `+ Capability` 添加 `App Groups`�?2. 新建一�?Group，例�?`group.com.trollstore.twitterdownloader`�?3. 同样地，选中 `TwitterShareExtension` target，也添加 `App Groups` 并勾选刚才创建的 `group`�?
## 步骤 3：替�?ShareViewController.swift
�?Xcode 为你生成�?`TwitterShareExtension/ShareViewController.swift` 的内容全部替换为以下原生代码�?
```swift
import UIKit
import Social
...
class ShareViewController: UIViewController {
    ...
    let appGroupId = "group.com.trollstore.twitterdownloader" // 请替换为你申请的 App Group ID
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = .clear // 透明背景，实现静默感
               ...
               let result = tweetResult["result"] as? [String: Any] {
                ...
                if let extended = legacy?["extended_entities"] as? [String: Any],
                   let medias = extended["media"] as? [[String: Any]] {
                    ...
                    if savedCount > 0 {
                        showToast("�?成功下载 \(savedCount) 张原�?)
                    } else {
                        showToast("未找到图�?)
                    }
                }
            } else {
                showToast("解析推文失败�?Cookie 过期")
            }
        } catch {
            showToast("网络请求失败")
        }
    ...
    func showToast(_ message: String) {
        // �?Extension 中可以使�?UNUserNotificationCenter 发送本地通知作为提示
        // 为了简便，这里直接打印。在真实设备上应实现通知推�?        print(message)
    }
...
## 步骤 4：在 Flutter 中适配 App Group
打开 Flutter 项目，建议使�?`shared_preference_app_group` 插件，或者如果你坚持使用 `shared_preferences`，需要确保存储的 domain 设置�?App Group �?ID，这�?Native 端才能通过 `UserDefaults(suiteName:)` 读到 `tw_auth_token`�?
